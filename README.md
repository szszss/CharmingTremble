...
- **Milestone2** *(已达成,v0.2版)* 增加分数和生命值显示,加入GameOver机制,加入字体渲染器.
- **Milestone3** *(已达成,v0.3版)*增加PowerUP和多种砖块机制.
- **Milestone4** *(已达成,v0.3版)*增加菜单和界面.
- **Milestone5** 增加配置系统,积分榜和录像回放,实现一个完整的游戏流程.
- **Milestone6** 优化以及必要的代码重构,补齐文档,实现Linux平台版本.
- **Milestone7** *(已达成,v0.3版)*订正Bug和完善美化(基本的玩家模型),没问题的话这个就已经是准release版了.
- **Milestone8** 更好的模型系统.
- **Milestone9** 联♂机!
...
*对面的那个天人凝视着地上的世界,即像是在思考,也像是在等待.我讨厌打断别人,但在窒息而死的危险下,我不得不打断那个中二少女的自我补完,肺里没有一丝空气的我一句话也说不出,不得不用洋伞的金属尖敲了敲地,我注意到伞比平时要格外地沉,就像握着一杆铁棍一样,毫无疑问这是缺氧的表现,一种麻痛感从指间传来,如野火似的向全身各处传播.就在我后悔自己赌气冒然跟她来到这里,并为愈发接近的死亡而感到紧张时,她终于听见了我的伞尖敲击要石的声音,转过了头,面带不屑地说:*
...
*"咳咳,好了,看你也快不行了,我们还是赶快说正事吧,我可不想给一个老太婆做CPR啊,"她不紧不慢地走到了她的那块要石的边缘,弯下腰一手扶着膝盖,一手指向地面.我蹒跚地挪步到要石边,向下望去,无数的砖块状的东西漂浮在空中,最近的离我似乎有几百米的距离,最远的,似乎远在地面,甚至,地下...*
...
编译方法
------
待补充...目前VS2010打开项目文件后,将”项目属性-配置属性-调试-工作目录”设为” $(TargetDir)”,然后就可以编译了.

顺便一提,你的VS版本最低必须为VS2010SP1,因为目前项目中使用到的#pragma execution_character_set("utf-8")是VS2010SP1后才有的.
...
更新日志
------
v0.3 - 2014-4-21 **(Milestone3)** **(Milestone4)** **(Milestone7)**
- 改善了字体渲染效果.
- 加入了属性系统(Attribute).
- 增加了多种图像载入支持.其中PNG和TGA使用LibPng和LibTga载入,JPG,BMP,PSD,GIF,HDR和PIC使用STB-Image载入.
- 增加了PMD模型载入和渲染.
- 增加了2种新砖块:石砖和青苔砖.
- 整理了VS项目结构.
- 增强Gui系统,引入界面.
...
- 增加玩家姓名和排行榜系统.
- 顶到游戏顶端也会掉血了.
- Debug模式下采用一个固定的质数作为随机数种,Release模式下使用时间作为随机数种.
- 修正了一些Bug...
- 还有一些更新,但我想不起来了...

v0.2 - 2014-1-4 **(Milestone2)**
- 大幅提升游戏流畅度.
- 降低了游戏对CPU的占用量.
- 一个简陋的字体渲染器原型被加入到游戏中,目前仍存在一些问题.
- Gui系统加入.
- 游戏架构开始为适应多人游戏而调整,但尚未完成.
- 增加了分数和生命的显示.
- 当玩家跌落到底部时会损失生命.
- 增加GameOver机制.
- 玩家无法再在跳跃时穿透砖块了.
- 稍稍加大了砖块的判定范围.
- 游戏内部强制使用UTF8编码,除了字体渲染器是使用UCS4编码.
- 统一源代码文件的编码为UTF8(有BOM),但愿没问题吧?(才怪呢,头文件有BOM是很糟糕的事情!)
- 修正了OpenGL的深度缓冲区的问题.
- 修正了砖块的渲染会比实际位置微微向左偏移半格的问题,在0.1版中,这个Bug很容易被误认为是砖块的左侧判定区过小而右侧判定区过大,实际上,观察开局时玩家脚下的那个砖块即可发现问题的真正所在.
- 修正了几个关于日志记录器的Bug.
- 修正了GameCrash函数不能正确结束游戏的Bug.
